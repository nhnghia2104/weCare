...
<body>
    ...
    <div class="d-flex">
        <div class="container pt-5 pb-5">
            <div class="row">
                <div class="col-md-6">
                    <img style="max-height:350px" class="d-block mt-auto mb-auto ml-auto mr-auto p_img" src="" alt="">
                </div>
                <div class="col-md-6 pl-4 pr-4">
                    <section class="product-sale-content ">
                        <h2 class="text-capitalize font-weight-lighter custom-font-1 p_style"></h2>
                            ...
                            <div class="container p-0">
                                <div class="row nameNprice">
                                    <div class="col-md-8 style">
                                        <h4 class="custom-font-5 text-uppercase p_name">
                                    ...
                                    <div class="col-md-4">
                                        <div class="float-right justify-content-center align-items-center">
                                            ...
                                            <h4 class='custom-font-2 custom-font-weight-light text-dec-line text-right text-uppercase p_oldPrice'>
                                            </h4>

                                            <h4 class="custom-font-5 text-right text-uppercase p_price">
                        ...
                        <!-- button add to cart -->
                        <button type="button" class="add-to-cart btn btn-info rounded-0 text-uppercase w-100 justify-content-center align-items-center" style="height:50px">
                                    ...
                                    <div class="container-fluid">
                                        <div class="row">
                                            <div class="col-lg-6 pr-5">
                                                <p class="custom-font-1 p_description">
    ...
    <script>
        ...
        function loadMainInfomation() {
            $.ajax({
                type: "POST",
                url: '<?php echo base_url(); ?>index.php/Product/productInfo',
                dataType: 'json',
                data: {
                    id: id,
                },
                success: function(responded) {
                    pushData(responded[0]);
                }
            });
        }

        function pushData(data) {
            $('.p_style').html(data.style);
            $('.p_name').html(data.Name);
            $('.p_oldPrice').html(data.discount > 0 ? ('$' + data.Price) : '')
            $('.p_price').html('&nbsp;$' + roundNumber(data.Price * (1 - data.discount)));
            $('.p_img').attr('src', data.Image);
            $('.p_description').html(data.Description);

            $('.add-to-cart').attr('data-id', data.ID);
            $('.add-to-cart').attr('data-name', data.Name);
            $('.add-to-cart').attr('data-price', data.Price);
            $('.add-to-cart').attr('data-image', data.Image);
            $('.add-to-cart').attr('data-discount', data.discount);

            detail = data.details;
            if (detail != '' && detail != '[]') {
                json = JSON.parse(detail);
                displayDetailContent(json);
            }
        }

        function displayDetailContent(content) {
            console.log(content);
            if (content != null) {
                innerhtml = "";
                for (index = 0; index < content.length; index++) {
                    ...
                    innerhtml += '<tr>' +
                        '<td class="text-left font-weight-light">' + content[index].key +
                        '</td>' +
                        '<td class="text-right">' + content[index].val +
                        '</td>' +
                        '</tr>';
                ...
                $.ajax({
                    ...
                    success: function(dataReturned) {
                        if (dataReturned.length > 0) {
                            ...
                            checkCustomerReview(dataReturned[0].id, id);
                        }
                    }
                });
            }
    ...
    <script>
        function checkCustomerReview(idCustomer, idProduct) {
            $.ajax({
                type: "POST",
                url: '<?php echo base_url(); ?>index.php/customer/account/checkCustomerBoughtProduct',
                dataType: 'json',
                data: {
                    idProduct: idProduct,
                ...
                success: function(responded) {
                    if (responded.status == 'bought') {
                        $('.writeReview_block').removeAttr('hidden');
                    }
                }
            });
        }
